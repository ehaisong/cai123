// 彩虹易支付（Epay / 13pay mapi）签名工具
// 协议：
//   1. 剔除 sign / sign_type / 空值
//   2. key ASCII 升序
//   3. k=v&k=v 拼接（不 urlencode）
//   4. 末尾直接追加商户密钥（不加 &）
//   5. MD5 小写
//
// 注意：workerd 的 crypto.subtle 不支持 MD5，这里用纯 JS 实现。
...
/** 字节级 MD5（UTF-8 输入，hex 小写输出） */
export function md5(s: string): string {
  // 把 UTF-8 字符串转成"每个字符 = 一个字节"的二进制字符串再喂给 md51
  const utf8 = unescape(encodeURIComponent(s));
  return hex(md51(utf8));
}

/** 彩虹易支付签名 */
export function signEpay(params: Record<string, string | number>, key: string): string {
