...
function Inner() {
  ...
  useEffect(() => {
    supabase.from("product_issues").select("*").eq("id", issueId).maybeSingle().then(({ data }) => {
  ...
  const submit = async () => {
    setLoading(true);
    const { error } = await supabase.from("product_issues").update({
  ...
  if (!form) return <div className="h5-shell"><PageHeader title="编辑期号" /><p className="p-4 text-sm text-muted-foreground">加载中…</p></div>;
  ...
  return (
    <div className="h5-shell flex min-h-screen flex-col">
      ...
      <main className="flex-1 px-3 py-3 space-y-3">
        ...
        <Field label="付费内容"><Textarea rows={5} value={form.paid_content} onChange={(e) => setForm({ ...form, paid_content: e.target.value })} /></Field>
        <Field label="状态">
        ...
        <Button className="w-full" size="lg" onClick={submit} disabled={loading}>{loading ? "保存中…" : "保存"}</Button>
      </main>
    </div>
  );
}
