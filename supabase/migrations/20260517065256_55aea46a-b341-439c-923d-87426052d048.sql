...
BEGIN
  ...
  WITH base AS (
    SELECT o.amount,
           round(o.amount * COALESCE(m.platform_rate, v_default_rate), 2) AS plat,
           o.paid_at
  ...
  FROM (
    SELECT m.id AS merchant_id,
           ...
           COALESCE(sum(round(o.amount * COALESCE(m.platform_rate, v_default_rate), 2)), 0) AS platform_amount
      FROM public.orders o
      JOIN public.merchants m ON m.id = o.merchant_id
     WHERE o.status = 'paid'
  ...
  FROM (
    SELECT o.id AS order_id,
           o.amount,
           round(o.amount * COALESCE(m.platform_rate, v_default_rate), 2) AS platform_amount,
           COALESCE(m.platform_rate, v_default_rate) AS rate,
           o.paid_at,
           o.merchant_id,
           m.shop_name,
